    ...
    async function iniciarJogo() {
        ...
        for (let i = 1; i < snake.length; i++) {
            if (snake[0].x == snake[i].x && snake[0].y == snake[i].y) {
                ...
                let response = await (
                    await (await fetch(apiUrl + "getcookie")).text()
                ).split("--user-name--");
                ...
                const uuid = await (
                    await fetch(apiUrl + "hash-to-uuid?hash", myConf)
                ).text();

                obj.uuid = uuid;
                obj.points = points;
                myConf.body = JSON.stringify(obj);
...
async function getMaximumPoints() {
    let response = await (
        await (await fetch(apiUrl + "getcookie")).text()
    ).split("--user-name--");
    ...
    const uuid = await (await fetch(apiUrl + "hash-to-uuid", myConf)).text();
