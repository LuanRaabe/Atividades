...
function createAccount(body) {
    ...
    try {
        ...
        (e) => console.log("Teve um erro aí ", e);
    }
}
...
function checkName(name) {
    ...
    let dataBase = require("./sessions.json");
    console.log(dataBase);
    let find = dataBase.filter((item) => item.name == name);
    console.log(find);
    return find[0].uuid;
}
...
function updatePoints(body) {
    let readDataBase = require("./sessions.json");
    let find = readDataBase
        .map((item, index) => {
            return item.uuid == body.uuid ? index : false;
        })
        .filter((el) => el !== false)[0];
    readDataBase[find].maxPoints =
        body.points > readDataBase[find].maxPoints
            ? body.points
            : readDataBase[find].maxPoints;
    let dataBase = "./sessions.json";
    try {
